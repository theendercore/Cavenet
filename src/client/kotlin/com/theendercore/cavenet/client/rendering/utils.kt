...
fun VertexConsumer.drawFaceFromDir(
    ...
    val sc = 0.01f
    val x1 = nodePos.x + (dir.normal.x * sc)
    val y1 = nodePos.y + (dir.normal.y * sc)
    val z1 = nodePos.z + (dir.normal.z * sc)
    ...
    when (dir) {
        Direction.UP ->
            face2(mtx, x1, y1, z1, x2, y1, z2, camPos, color, light, vec3(0, 1, 0), sprite)

        Direction.DOWN ->
            face2(mtx, x2, y2, z2, x1, y2, z1, camPos, color, light, vec3(0, 1, 0), sprite)

        Direction.SOUTH -> face(mtx, x2, y2, z1, x1, y1, z1, camPos, color, light, vec3(0, 1, 0), sprite)
        Direction.NORTH -> face(mtx, x1, y2, z2, x2, y1, z2, camPos, color, light, vec3(0, 1, 0), sprite)
        Direction.EAST -> face(mtx, x1, y2, z1, x1, y1, z2, camPos, color, light, vec3(0, 1, 0), sprite)
        Direction.WEST -> face(mtx, x2, y2, z2, x2, y1, z1, camPos, color, light, vec3(0, 1, 0), sprite)
    }
}

fun VertexConsumer.drawDoorFace(
    mtx: Matrix4f,
    nodePos: BlockPos,
    camPos: Vec3,
    color: Int,
    ...
    val y1 = nodePos.y + (dir.normal.y * -0.5)
    ...
    val z2 = z1 + 1
    when (dir) {
        Direction.DOWN -> face2(mtx, x1, y1, z1, x2, y1, z2, camPos, color, light, vec3(0, 1, 0), sprite)
        Direction.UP -> face2(mtx, x2, y2, z2, x1, y2, z1, camPos, color, light, vec3(0, 1, 0), sprite)
        Direction.SOUTH,
...
fun VertexConsumer.face2(
    ...
    addVertex(mtx, vec3(x2, y2, z1), camPos)
        .setColor(color)
        .setUv(sprite.u0, sprite.v1)
        .setOverlay(OverlayTexture.NO_OVERLAY)
        .setLight(light)
    ...
    addVertex(mtx, vec3(x1, y1, z2), camPos)
        .setColor(color)
        .setUv(sprite.u1, sprite.v0)
        .setOverlay(OverlayTexture.NO_OVERLAY)
        .setLight(light)
